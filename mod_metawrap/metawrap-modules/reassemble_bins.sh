...
if [ "$run_checkm" = true ]; then
	...
	rm -r ${out}/tmp
        ...
        checkm nx_plot -x fa ${out}/reassembled_bins.checkm ${out}/reassembled_bins ${out}/reassembled_bins.plot
        if [[ ! -s ${out}/reassembled_bins.plot/nx_plot.png ]]; then warning "Something went wrong with making the CheckM plot. Exiting."; fi
        mv ${out}/reassembled_bins.plot/nx_plot.png ${out}/reassembled_bins.png

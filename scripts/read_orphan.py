...
def filter_for_orphans(pair_0_path_i, pair_1_path_i, orphans_path_i, pair_0_path_o, pair_1_path_o, unique_path_o):
    ...
    df_0["ID"] = df_0["ID"].str.split(" ").str[0]
    df_1["ID"] = df_1["ID"].str.split(" ").str[0]
    ...
    print(dt.today(), "DF0:", df_0["ID"])
    print(dt.today(), "DF1:", df_1["ID"])
    ...
    if(os.path.exists(orphans_path_i)):
        if(os.path.getsize(orphans_path_i)> 100):
            print(dt.today(), "orphanizer -> appending to existing S")
...
if __name__ == "__main__":
    if(len(sys.argv) < 7):
        ...
        pair_0_path_i = os.path.abspath(sys.argv[1])
        pair_1_path_i = os.path.abspath(sys.argv[2])
        ...
        print(dt.today(), "start orphanizer")
